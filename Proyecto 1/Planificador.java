...
//******************************************************************************************
	public static void main(String[] args) {
		int tam, quantum, fin = 0, q_aux;
		int espacio, t_ejec = 0, num_proc = 0;
		int t_ejecp = 0, t_espp = 0;
...
//SE CARGAN UNO POR UNO LOS PROCESOS DE LA COLA DE PROCESOS LISTOS A LA COLA DE EJECUCION
				while (proclistos.GetH() != null){
					while ( procejec.ValidaEspacio() == true /*&& proclistos.GetH() != null*/ ) {
						...
						if ( espacio <= 0 ) { //SI LA MEMORIA ES INSUFICIENTE MANDA EL MENSAJE CORRESPONDIENTE
					...
					System.out.println("\n>>>>>>>>>>ORDENANDO<<<<<<<<<<");
					procejec.SortIntercambio();
					procejec.Listar();
					System.out.println(">>>>>>>>>>FIN DE ORDENAMIENTO<<<<<<<<<<\n");

					while ( procejec.ValidaVacio() == false ) {
						aux = procejec.Borrar();
						System.out.println ("\nSe cargo el proceso:" + aux.GetID() + 
											"\t||Nombre: " + aux.GetNombre() + 
											"\t||Tama\u00f1o: " + aux.GetTamanio() + 
											"\t||Restan: " + espacio + " de memoria");
						aux.LeeTInic(t_ejec);
						...
						while ( aux.GetTiempo() != 0 ) {
							...
							/*aqui puede estar el error*/	aux = proclistos.Borrar().GetInfo(); //EXTRAE EL SIGUIENTE PROCESO A EJECUTAR
								procejec.Insertar(aux);			//INSERTA EL PRIMER PROCESO EN LA COLA DE PROCESOS LISTOS PARA EJECUCUION
								espacio = espacio - aux.GetTamanio(); //RESTA EL TAMAÑO DEL PROCESO DEL TAMAÑO DE LA MEMORIA
							}
						}						
